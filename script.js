...
const setupGrid = (letterArr, gridName, labelArr, parentGridDiv) => {
  ...
  for (let i = 10; i >= 0; i--) {
    ...
    newDiv.setAttribute(
      "class",
      `${gridName}__number-labels ${gridName}__label-${i}`
    );
    if (i != 0) {
      newDiv.innerHTML = i;
    }
    parentGridDiv.appendChild(newDiv);
  }
  ...
  for (let i = 9; i >= 0; i--) {
    ...
    newLabel.setAttribute(
      "class",
      `${gridName}__labels ${gridName}__label-${letterArr[i]}`
    );
...
let gameLogPlayer = document.querySelector(
  ".battleship-game__info__log-content-player"
);
let gameLogOpponent = document.querySelector(
  ".battleship-game__info__log-content-opponent"
);
...
const startGameButton = document.querySelector(
  ".battleship-game__info__start-game-button"
);
...
const generateGameComponents = () => {
  ...
  let playerShipsIdArray = [...playerShips].map((ship) => {
    return ship.id;
  ...
  let opponentBoardIdArray = [...allGuessButtons].map((button) => {
    return button.id;
  });
...
const gameLogic = (gameComponents) => {
  allGuessButtons.forEach((coordinate) => {
    coordinate.addEventListener("click", (e) => {
      handlePlayerGuess(
        gameComponents.opponentShipsIdArray,
        e.target.id,
        e.target.classList
      ); // Handle player guess
      setTimeout(
        handleOpponentGuess(
          gameComponents.playerShipsIdArray,
          gameComponents.playerBoardIdArray
        ),
        1000
      );
...
const handlePlayerGuess = (shipsArr, id, classList) => {
  ...
  if (shipsArr.length == 0) {
    gameLogPlayer.innerHTML =
      "<h3> Well Done. You have won Battleship! <br> Your grand prize is: Nothing. </h3>";
...
const handleOpponentGuess = (shipsArr, playerBoardArray) => {
  ...
  let randomOpponentGuessIndex = Math.floor(
    Math.random() * playerBoardArray.length
  );
  ...
  if (isHit) {
    ...
    document
      .querySelector(`#${randomOpponentIdGuess}`)
      .classList.toggle("ship-guess-choice-success");
  ...
  if (shipsArr.length == 0) {
    gameLogOpponent.innerHTML =
      "<h3> Oh no! All your ships are sunk and you have lost Battleship. <br> Better luck next time! </h3>";
    gameLogOpponent.style.color = "red";
    gameLogPlayer.innerHTML = "";
    playerGrid.style.visibility = "hidden";
    opponentGrid.style.visibility = "hidden";
